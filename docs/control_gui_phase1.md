...
mock bring-up without RViz, runs core and offscreen GUI tests, exercises both arms
...
## Phase 2 Cartesian pose preview

The GUI Cartesian pose tab supports one arm at a time. `Copy current TCP` reads the
gateway's fresh `world -> left_flange/right_flange` TF state. Targets use metres and a
normalized quaternion. Validate checks ownership, arm/TCP pairing, timestamp, TF,
finite values and workspace guard; Plan adds MoveIt position/orientation constraints
for collision-aware IK/planning; Execute dispatches only the stored approved plan.

The same tab provides press-and-hold world-frame `X/Y/Z/Rx/Ry/Rz` jog. Linear speed is
...
`operating_mode=mock` enables validated writes. Every acquisition/write is rejected by
the backend in `physical` mode during Phase 1, regardless of GUI state. Future RL and
teleoperation nodes should implement the same lease/heartbeat/action sequence; they do
not receive a privileged controller path. Cartesian jog, MoveIt Servo, physical writes,
force/torque, GPIO, and remote/browser clients remain outside Phase 1.
